...
public class RoadActor extends UntypedActor {
	 ...
	 */
	static HashMap<Id<Link>, RoadActor> allRoads = null;

	public static HashMap<Id<Link>, RoadActor> getAllRoads() {
		return allRoads;
	}

	public static void setAllRoads(HashMap<Id<Link>, RoadActor> allRoads) {
		RoadActor.allRoads = allRoads;
	}
	...
	public RoadActor(ActorRef scheduler, Link link, ActorRef messageHandlerActor) {
		this.scheduler = scheduler;
		this.messageHandlerActor=messageHandlerActor;
		this.link = link;
	...
	private void unscheduleDeadlockPreventionMessage(DeadlockPreventionMessage m){
		messageHandlerActor.tell(m, getSelf());
	}
	...
	public void leaveRoad(Vehicle vehicle, double simTime) {
		...
		if (this.interestedInEnteringRoad.size() > 0) {
			...
			assert (m.vehicle == nextVehicle); // TODO: think through, if this might be a problem or not with Akka actors
			unscheduleDeadlockPreventionMessage(m);
			...
			scheduleEnterRoadMessage(nextAvailableTimeForEnteringStreet, getRoad(),nextVehicle);
		} else {
			if (this.gap != null) {
		...
		if (this.carsOnTheRoad.size() > 0) {
			...
			nextVehicle.scheduleEndRoadMessage(nextAvailableTimeForLeavingStreet, getRoad());
		}
	...
	public void enterRoad(Vehicle vehicle, double simTime) {
		...
		if (this.carsOnTheRoad.size() == 1) {
			nextAvailableTimeForLeavingStreet = Math.max(nextAvailableTimeForLeavingStreet,
					this.timeOfLastLeavingVehicle + this.inverseOutFlowCapacity);
			vehicle.scheduleEndRoadMessage(nextAvailableTimeForLeavingStreet, getRoad());
//		} else { // empty else clause
	...
	public void enterRequest(Vehicle vehicle, double simTime) {
		...
		if (this.carsOnTheRoad.size() + this.noOfCarsPromisedToEnterRoad < this.maxNumberOfCarsOnRoad) {
			...
			scheduleEnterRoadMessage(nextAvailableTimeForEnteringStreet, getRoad(),vehicle);
		} else {
			/*
	...
	public void handleDeadlockPreventionMessage(DeadlockPreventionMessage deadlockPreventionMessage){
		incrementPromisedToEnterRoad(); // this will be decremented in
												// enter road
		setTimeOfLastEnteringVehicle(deadlockPreventionMessage.getMessageArrivalTime());
		removeFirstDeadlockPreventionMessage(deadlockPreventionMessage);
		removeFromInterestedInEnteringRoad();

		scheduleEnterRoadMessage(deadlockPreventionMessage.getMessageArrivalTime(),getRoad(),deadlockPreventionMessage.vehicle);
	}
	
	public Road getRoad(){
		return getRoad(getLink().getId());
	}
	
	public void scheduleEnterRoadMessage(double scheduleTime, Road road, Vehicle vehicle) {
		 ...
		 * (which has index -1)
		 */
		if (vehicle.getLinkIndex() >= 0) {
			vehicle.scheduleLeavePreviousRoadMessage(scheduleTime);
		}
		...
		if (vehicle.isEndingLegMode()) {
			 ...
			 */
			giveBackPromisedSpaceToRoad(); // next road
			vehicle.scheduleEndLegMessage(scheduleTime, road);
		} else {
			vehicle._scheduleEnterRoadMessage(scheduleTime, road);
		}
	}
